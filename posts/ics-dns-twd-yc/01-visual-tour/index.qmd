...
To match the ICS calibration document's data specification (§88), the
...
This yields **824 weekly observations**, one for each ISO calendar
week in the range --- 100% ISO-week coverage. Of these, 752 fall on
Fridays. Earlier weekdays appear when Friday is a holiday --- most
commonly during Lunar New Year week. A small number of observations
fall on a Saturday: Taiwan occasionally schedules a make-up trading
day on a Saturday to compensate for a weekday holiday adjacent to a
long break. These are legitimate trading days and are included as
such.
...
Static charts can show one or two of these features at a time. The
animation shows all three at once.
...
- The 10-year rate has not exceeded 1.91% in this sample. For an
  insurance industry that historically priced guaranteed-return
  products off rate assumptions well above this, the implications for
  in-force books are significant.
- Mean yields rise nearly linearly across tenors from 0.69% at 1Y to
  1.21% at 10Y. The standard deviation also rises with tenor, from
  0.32% to 0.40% --- longer tenors are modestly more volatile in
  absolute terms.
- The dispersion within any single tenor (standard deviation of around
  0.3%--0.4%) is comparable to the dispersion across tenors at any
  single date (10Y mean minus 1Y mean is 0.51%). This means level
  shifts and slope shifts are both first-order drivers of curve
  variation. Neither dominates the other. Article 2 will revisit this
  observation through principal component analysis.
...
Specific dates from the sample will reappear in the later articles when
the calibration and stress scenarios are discussed. The most useful to
fix in mind:
...
The 10-year rate spent approximately three years (2019--2021) below 1%,
returned to roughly its 2010 level by 2024, and has been broadly stable
since. The DNS model that this series calibrates in Article 4 will
treat the full sixteen-year window as one stationary process; whether
that is a reasonable treatment, given the visible regime structure, is
a question worth coming back to once the model has been estimated.
...
- Yields move together across tenors, but not identically. Both level
  and shape change over time. **Article 2 (PCA)** quantifies how much
  of the variation lies in each direction.
- Volatility increases with tenor in absolute terms. The model in
  **Article 3 (DNS framework)** allows different factors to have
  different volatilities, which is consistent with this observation.
- The data covers a low-rate regime, a tightening cycle, and a
  post-tightening reset. **Article 4 (Kalman calibration)** estimates
...
The next article begins the analytical work proper, with a principal
